...
import { ExtractedVarietyPreview } from '../../types';
import { X, CheckSquare, Square, ShieldCheck, AlertCircle, ArrowRightCircle } from 'lucide-react';
...
interface VarietyValidationModalProps {
  ...
  onClose: () => void;
  onSubmit: (selectedVarieties: ExtractedVarietyPreview[]) => Promise<void> | void;
  isSubmitting?: boolean;
}

type SelectableVariety = ExtractedVarietyPreview & { _selectionKey: string };
...
const VarietyValidationModal: React.FC<VarietyValidationModalProps> = ({
  ...
  const filteredVarieties: SelectableVariety[] = useMemo(() =>
    selectableVarieties.filter(variety => {
