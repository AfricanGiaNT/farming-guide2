#!/usr/bin/env python3
"""
Ginger Section Detailed Analysis
Examine ginger section to find variety tables, management info, and section structure
"""
...
def analyze_ginger_section():
    """Analyze ginger section in detail"""
    
    print("=" * 80)
    print("DETAILED GINGER SECTION ANALYSIS")
    print("=" * 80)
    
    with pdfplumber.open(PDF_PATH) as pdf:
        # First, let's find the ginger section by searching for section 3.9.3
        ginger_pages = []
        
        # Search through pages to find ginger section
        for page_num in range(250, 300):  # Search pages 250-300
            ...
            # Look for ginger section markers
            if '3.9.3' in text and ('ginger' in text.lower() or 'zingiber' in text.lower()):
                ginger_pages.append(page_num + 1)
                print(f"Found ginger section on page {page_num + 1}")
        
        print(f"\nGinger section found on pages: {ginger_pages}")
        
        # Analyze each ginger page
        for page_num in ginger_pages:
            page_idx = page_num - 1
            ...
            management_keywords = ['fertilizer', 'planting', 'field prep', 'mulching', 'pest', 'disease', 'harvesting', 'yield']
            management_lines = []
