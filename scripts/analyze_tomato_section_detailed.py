#!/usr/bin/env python3
"""
Tomato Section Detailed Analysis
Examine tomato section to find variety tables, management info, and section structure
"""
...
def analyze_tomato_section():
    """Analyze tomato section in detail"""
    
    print("=" * 80)
    print("DETAILED TOMATO SECTION ANALYSIS")
    print("=" * 80)
    
    with pdfplumber.open(PDF_PATH) as pdf:
        # First, let's find the tomato section by searching for section 3.10.4
        tomato_pages = []
        
        # Search through pages to find tomato section
        for page_num in range(400, 500):  # Search pages 400-500
            ...
            # Look for tomato section markers
            if '3.10.4' in text and ('tomato' in text.lower() or 'lycopersicon' in text.lower()):
                tomato_pages.append(page_num + 1)
                print(f"Found tomato section on page {page_num + 1}")
        
        print(f"\nTomato section found on pages: {tomato_pages}")
        
        # Analyze each tomato page
        for page_num in tomato_pages:
            ...
            management_keywords = ['fertilizer', 'transplanting', 'spacing', 'pest', 'weed', 'control']
            management_lines = []
