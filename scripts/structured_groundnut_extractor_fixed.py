#!/usr/bin/env python3
"""
Structured Groundnut Variety Extractor - Fixed Version
Extract groundnut varieties using only existing columns first, then update with additional info
"""
...
class GroundnutVarietyExtractor:
    ...
    def insert_groundnut_varieties(self, varieties: List[Dict], fertilizer_info: str, pest_info: str, disease_info: str):
        ...
        for variety in varieties:
            try:
                # Prepare variety data using only existing columns
                variety_data = {
                ...
                additional_info = []
                if variety.get('botanical_type'):
                    additional_info.append(f"Botanical Type: {variety['botanical_type']}")
                if variety.get('spacing_info'):
                    additional_info.append(f"Spacing: {variety['spacing_info']}")
                if variety.get('seed_rate_info'):
                    additional_info.append(f"Seed Rate: {variety['seed_rate_info']}")
                if variety.get('special_notes'):
                    additional_info.append(f"Details: {variety['special_notes']}")
                
                if additional_info:
                    variety_data['special_characteristics'] = '; '.join(additional_info)
    ...
    def extract_all_groundnut_varieties(self):
        ...
        all_varieties = varieties_table30 + varieties_recommended
